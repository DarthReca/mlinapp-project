...
class CelebADataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        if (torchvision_class):   
            ### Use the torchvision class
            if stage == "fit" or stage is None:
                if self.indices is not None:
                    self.train_dataset = Subset(
                        torchvision.datasets.CelebA("data", split="all", transform=self.transform), self.indices
                    )
                else:
                ...
                self.val_dataset = Subset(
                    torchvision.datasets.CelebA("data", split="valid", transform=self.transform),
                    torch.arange(10),
                )
            ...
            if stage == "fit" or stage is None:
                self.train_dataset =  CelebA(self.data_path, self.attr_path, "train", self.selected_attrs, self.transform)

                self.val_dataset =  CelebA(self.data_path, self.attr_path, "val", self.selected_attrs, self.transform)
                
            if stage == "test" or stage is None:
                self.test_dataset = CelebA(self.data_path, self.attr_path, "test", self.selected_attrs, self.transform)
...
class CelebA(Dataset):
    ...
    def __getitem__(self, index):
        ...
        att = torch.tensor((self.labels[index] + 1) // 2) # attributes values shifted from -1/1 to 0/1
        return img, att
