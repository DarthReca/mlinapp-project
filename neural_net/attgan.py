...
class AttGAN(pl.LightningModule):
    ...
    def training_step(
        ...
        fake_attributes_b = orig_attributes_a[permuted_indexes].contiguous()  # fake attributes

        fake_attributes_b = fake_attributes_b.float()
        orig_attributes_a = orig_attributes_a.float()

        shifted_orig_attributes_a_tilde = (orig_attributes_a * 2 - 1) * self.thres_int  # orig_attributes_a shifted to -0.5,0.5

        if self.b_distribution == "none":
            shifted_fake_attributes_b_tilde = (fake_attributes_b * 2 - 1) * self.thres_int

        if self.b_distribution == "uniform":
            shifted_fake_attributes_b_tilde = (fake_attributes_b * 2 - 1) * torch.rand_like(fake_attributes_b) * (2 * self.thres_int)
        ...
        if optimizer_idx == 0:
            ...
            reconstructed_images = self.generator(embedding_zs_a, shifted_orig_attributes_a_tilde, mode="dec")
            ...
            fakes_discrimination, fakes_classification = self.discriminators(fake_images)
            ...
            d_loss = self.discriminators_loss(fakes_classification, fake_attributes_b.float())
            # Adversarial loss (generator) -> how much the discriminator is been fooled predicting "real" when the images were actually fake
            a_loss = self.adversarial_loss(fakes_discrimination, torch.ones_like(fakes_discrimination))
        ...
        if optimizer_idx == 1:
            ...
            fake_images = self.generator(orig_images_a, shifted_fake_attributes_b_tilde, mode="enc-dec").detach()
            # 2) The discriminator gets as input the real images, saying if they are real/fake and predicting their attributes
            reals_discrimination, reals_classification = self.discriminators(orig_images_a)
    ...
    def validation_step(self, batch, batch_idx: int):
        ...
        fake_images = (self.generator(orig_images, target) * 255).round().byte()
        self.metrics.update(fake_images)
