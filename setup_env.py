...
if sys.version_info.major != 3 or sys.version_info.minor != 9:
    print(f"{bcolors.WARNING}Warning: the project was developed using Python 3.9, but you are using Python {version} consider switching to that version if you encounter problems when running the scripts{bcolors.ENDC}")
else:
...
with open("requirements.txt") as reqfile:
    for line in reqfile.readlines():
        ...
        if package not in installed_packages:
            print(f"{bcolors.WARNING}Package '{package}' is not installed{bcolors.ENDC}")
            missing_packages = True
        else:
            print(f"{bcolors.SUCCESS}Package '{package}' is already installed{bcolors.ENDC}")
...
if missing_packages:
    if utils.query_yes_no("Some packages are missing.\nWould you like to run 'pip install -r requirements.txt' now?", default=None):
        ...
        if result == 0:
            print(f"{bcolors.SUCCESS}Requirements successfully installed ✔{bcolors.ENDC}")
            missing_packages = False
        else:
            print(f"{bcolors.ERROR}Something went wrong while installing requirements, please try again later ✘{bcolors.ENDC}")
            errors = True
else:
...
if os.path.isfile("data/celeba/img_align_celeba.zip"):
    ...
    if utils.query_yes_no("Would you like to download the dataset now?", default=None):
        try:
            urllib.request.urlretrieve("https://www.dropbox.com/s/ydfwka7plnrd7dz/img_align_celeba.zip?dl=1", "data/celeba/img_align_celeba.zip", utils.reporthook)
            print(f"{bcolors.INFO}Download completed{bcolors.ENDC}")
        except KeyboardInterrupt:
            print(f"{bcolors.WARNING}Download stopped, please delete the partial dataset before attempting again{bcolors.ENDC}")
...
if os.path.isfile("data/celeba/img_align_celeba.zip"):
    ...
    if computed_md5 != correct_md5:
        ...
        print(f"{bcolors.SUCCESS}The dataset passed the integrity check ✔{bcolors.ENDC}")
...
if os.path.isfile("weights/pretrained.pth"):
    ...
    if utils.query_yes_no("Would you like to download the weights now?", default=None):
        try:
            urllib.request.urlretrieve("https://www.dropbox.com/s/44g36acwkxlsllw/inject0_orig.pth?dl=1", "weights/pretrained.pth", utils.reporthook)
            print(f"{bcolors.INFO}Download completed{bcolors.ENDC}")
        except KeyboardInterrupt:
            print(f"{bcolors.WARNING}Download stopped, please delete the partial weights before attempting again{bcolors.ENDC}")
...
if os.path.isfile("weights/pretrained.pth"):
    ...
    if computed_md5 != correct_md5:
        ...
        print(f"{bcolors.SUCCESS}The weights passed the integrity check ✔{bcolors.ENDC}")
...
print('\n== Setup ended ==')
if errors:
    print(f"{bcolors.ERROR}The setup finished with errors ✘\nPlease try again{bcolors.ENDC}")
else:
    if any([missing_packages, missing_dataset, missing_weights]):
        print(f"{bcolors.WARNING}The setup finished without errors, but you are missing:")
        if missing_packages: print("• some packages")
        if missing_dataset: print("• the dataset")
        if missing_weights: print("• the pretrained weights")
        print(f"Consider running this script again{bcolors.ENDC}", sep="")
    else:
        print(f"{bcolors.SUCCESS}Everything went smoothly, you are ready to get training ✔{bcolors.ENDC}")